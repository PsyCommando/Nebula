...
/obj/machinery/embedded_controller/radio
	...
	var/tmp/screen_state    = "screen_standby"                ///Icon state of the screen used by dummy controllers to match the same state
	var/tmp/indicator_state = 0                               ///Bitflag to indicate which indicator lights are on so dummy controllers can match the same state
	var/obj/machinery/embedded_controller/radio/master        ///If set, this controller will route its commands to the master controller with the same id_tag.
	var/datum/radio_frequency/radio_connection

	//Indicator flags
	var/static/const/INDICATOR_FLAG_DONE   = BITFLAG(1)       ///"indicator_done"
	var/static/const/INDICATOR_FLAG_ACTIVE = BITFLAG(2)       ///"indicator_done"
	var/static/const/INDICATOR_FLAG_FORCED = BITFLAG(3)       ///"indicator_done"
